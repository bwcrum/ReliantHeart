...
public class OTA_Object_UInt_Array extends OTAObject implements iOTAMessage {
	
	public OTA_Object_UInt_Array (int objectid) throws Exception {
		
		super(objectid, iOTAMessage.OBJTYPE_ARRAY_UINT);
		obj_data = new long[0];
		resize();
		
	}
	
	public OTA_Object_UInt_Array(int objectid, long[] data) throws Exception {
		
		super(objectid, iOTAMessage.OBJTYPE_ARRAY_UINT);
		setValue(data);
	}
	
	public OTA_Object_UInt_Array(int objectid, int[] data) throws Exception {
		
		super(objectid, iOTAMessage.OBJTYPE_ARRAY_UINT);
		setValue(data);
	}
	
	public OTA_Object_UInt_Array(int objectid, short[] data) throws Exception {
		
		super(objectid, iOTAMessage.OBJTYPE_ARRAY_UINT);
	    setValue(data);
	}
	
	public OTA_Object_UInt_Array(int objectid, byte[] data) throws Exception {
	...
	public OTA_Object_UInt_Array(byte[] incoming) throws Exception{
	...
	public String toString() {
		...
		for(int j=0; j<obj_data.length; j++) {
			...
			{
			case 1:
				num = new Short((short) (obj_data[j] & 0xFF));
				break;
			case 2:
				num = new Integer((int) (obj_data[j] & 0xFFFF));
				break;
			case 4:
				num =  new Long((long) (obj_data[j] & 0xFFFFFFFFL));
				break;
			case 8:
				/*
				BigInteger TWO_64 = BigInteger.ONE.shiftLeft(64);
				BigInteger b = BigInteger.valueOf(obj_data[i]);
				   if(b.signum() < 0) {
				      b = b.add(TWO_64);
				   }
				num = b;
	...
	public Object getValue() {
		...
		for(int i=0; i<obj_data.length; i++)
		{
			...
			{
			case 1:
				num = new Short((short) (obj_data[i] & 0xFF));
				break;
			case 2:
				num = new Integer((int) (obj_data[i] & 0xFFFF));
				break;
			case 4:
				num =  new Long((long) (obj_data[i] & 0xFFFFFFFFL));
				break;
			case 8:
				/*
				BigInteger TWO_64 = BigInteger.ONE.shiftLeft(64);
	...
	static public void main (String[] args){
		...
		int oid = 50;
		OTA_Object_UInt_Array obj = null;
		try {
			obj = new OTA_Object_UInt_Array (oid++);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			obj = new OTA_Object_UInt_Array(oid++, data3);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		OTA_Object_UInt_Array obj4 = obj;
		...
		try {
			obj = new OTA_Object_UInt_Array(data4);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		try {
			obj = new OTA_Object_UInt_Array(obj4.getBytes());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
