...
public abstract class OTAObject implements iOTAMessage {
	...
	protected String strObjectType() {
		...
		case OBJTYPE_ARRAY_FLOAT:
			...
			break;		
		case OBJTYPE_UBYTE:
			strObjType = "UNSIGNED BYTE";
		break;
		case OBJTYPE_UINT:
			strObjType = "UNSIGNED INT";
		break;
		case OBJTYPE_ARRAY_UBYTE:
			strObjType = "ARRAY OF UNSIGNED BYTE";
		break;
		case OBJTYPE_ARRAY_UINT:
			strObjType = "ARRAY OF UNSIGNED INT";
		break;
	...
	static public OTAObject constructFromInputStream (DataInputStream input) throws Exception{
		...
		try {
			...
			case iOTAMessage.OBJTYPE_UBYTE:
				int data =  input.read();
				if(data == -1)
			...
			case iOTAMessage.OBJTYPE_ARRAY_UBYTE:
				read = input.read(size_2);
				if(read == -1 || read < 2)
			...
			case iOTAMessage.OBJTYPE_ARRAY_UINT:
				read = input.read(size_2);
				if(read == -1 || read < 2)
